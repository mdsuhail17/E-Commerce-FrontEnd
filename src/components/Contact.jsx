...
export default function Contact() {
  ...
  const actionData = useActionData(); // for business logic
  const formRef = useRef(null);  // to refer the form in actual dom
  const navigation = useNavigation(); // to know the state of the form(submitting,loading,idle)
  const submit = useSubmit(); // to submit the form programmatically 

   const isSubmitting = navigation.state === "submitting";

  useEffect(()=>{
if(actionData?.success){
  formRef.current?.reset();  // reset the from inactual dom, this  current?.reset() directly manupulatin the actual dom so its doest not rerender
  ...
  return (
    <div className="max-w-[1152px] min-h-[852px] mx-auto px-6 py-8 font-primary bg-normalbg dark:bg-darkbg">
      ...
      {/* Contact Form */}
      <Form ref={formRef} method='POST' onSubmit={handleSubmit} className="space-y-6 max-w-[768px] mx-auto"
      >
        {/* Name Field */}
        <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <p className="text-red-500 text-sm mt-1">
              {actionData.errors.mobileNumber}</p>
          )}
          </div>
        </div>
...
export async function contactAction({request, params}){
  ...
  const contactData ={
    ...
    throw new Response(
      error.Response?.data?.errorMessage|| error.message || "Failed to Submit Ypur message. Please try again.",
